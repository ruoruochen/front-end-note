...
`Loader` 就是加载器，Loader的作用是让webpack拥有了加载和解析非JavaScript文件的能力。 因为webpack原生是只能解析js文件， Loader 充当翻译官的身份，对其他类型的资源进行转译工作。
...
浏览器是否使用缓存、缓存多久，是由服务器控制，服务器发回的响应的「响应头」部分的某些字段指明了有关缓存的关键信息，比如Cache-Control(控制缓存的行为)等等。
