...
HTTP协议采用明文传输信息，存在信息窃听、篡改和劫持的风险。HTTPS比HTTP多了一层TLS/SSL协议，而协议TLS/SSL具有身份验证、信息加密和完整性校验的功能，可以避免此类问题发生。http默认端口80

HTTPS是在HTTP上建立SSL加密层，对传输数据进行加密，是HTTP协议的安全版。https默认端口443
...
http1.0的缓存策略主要是Last-Modified/If-Modified-Since,Expire(到期)

http1.1的缓存策略，增加了 Etag / If-None-Match、Cache-Control等。
