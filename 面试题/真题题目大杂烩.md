...
![image-20210406141117834](C:%5CUsers%5CAsus%5CAppData%5CRoaming%5CTypora%5Ctypora-user-images%5Cimage-20210406141117834.png)
...
- 如果四种匹配均未成功，则分成两种情况考虑：
  - 新旧节点存在key，则会根据OldChild的key生成一张hash表，用newStart的key与hash表做匹配，匹配成功，就判断是否为sameNode，是，则就在真实DOM中将成功的节点移到最前面；否则，将newStart生成对应的节点插入到dom中对应的oldStart位置，S指针向中间移动，被匹配到old中的节点置null。
  - 如果没有key,则直接将`S`生成新的节点插入真实DOM。
