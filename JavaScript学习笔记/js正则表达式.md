...
`[\s\S]  [\w\W]`这样的用法，比较`"."`所匹配的还要多，因为**`"."`是不会匹配换行的**，所有出现有换行匹配的时候，人们就习惯使用`[\s\S]`或者`[\w\W]`这样的完全通配模式。
...
可以想象成是变量，回溯引用的语法像`\1`,`\2`,....,其中`\1`表示引用的第一个子表达式，`\2`表示引用的第二个子表达式，以此类推。而`\0`则表示整个表达式。

**练习：匹配文本中两个连续相同的单词**

```js
const str2 = 'Hello what what is the first thing, and I am am scq000.'
const pattern2 = /\b(\w+)\s\1/g
console.log(str2.match(pattern2)) //[ 'what what', 'am am' ]
```

##### 非捕获正则

不想子表达式被引用，则使用非捕获正则`(?:regex)`

```js
var str4 = 'scq000'
console.log(str4.replace(/(scq00)(?:0)/, '$1,$2')) //scq00,$2
```
...
**向前查找：**限制后缀。以`(?=regex)`在匹配时会限制前面的表达式匹配。
